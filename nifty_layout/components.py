""" Spiffy Report Components

A micro-framework for defining re-usable report components from the fields of an object

- a "Report Component" has 2 parts;
  - Component Nodes define the hierarchical structure for a sub-set of report fields.
    Each node encapsulates how to access, format, and label a field's value and its children.
    Once bound to a data object (e.g., a report model instance) the "bound node" provides a
...
- most components expect to work with django Models,
...
from typing import TypeAlias, Any, Callable, Optional, Type, Hashable, Iterator

from django.db import models
from django.template import Template
from django.template.loader import get_template

from .accessor import AccessorSpec, AccessorType, Accessor, get_accessor
...
def get_formatted_labeller(labeller, formatter):  # todo - add tests or change labelling/formatting logic
...
class BoundNode(Iterable):
    ...
    def __init__(self, data: Any, node: BaseNode,
...
class BoundSequenceNode(BoundNode, Sequence):
    ...
    def __init__(self, data: Any, node: SequenceCompositeNode,
                 template: Optional[TemplateType] = None):
...
class BoundDictNode(BoundNode, Mapping):
    ...
    def __init__(self, data: Any, node: DictCompositeNode,
                 template: Optional[TemplateType] = None):
        self.node = node   # only to please the type checker, which otherwise thinks node is a BaseNode
...
class BaseNode(Iterable, abc.ABC):
    ...
    bound_node_type: type[BoundNode] = BoundNode      # data type returned by `bind` operation
    ...
    def __init__(self,
        ...
        self.formatter = formatter if formatter is not None else type(self).default_formatter
        self.labeller = labeller if labeller is not None else type(self).default_labeller
    ...
    def get_label(self, data: Any) -> str | None:
        ...
        return (
            self.labeller(data) if callable(self.labeller) else
            str(self.labeller) if self.labeller else None
        )
...
class FieldNode(AtomicNode):
    ...
    default_accessor_type: Type[AccessorType] = Accessor  # The Accessor type used to wrap naked accessor spec.
    default_labeller: Labeller | str | bool = field_labeller

    def __init__(self, accessor: AccessorSpec,
                 formatter: Optional[Formatter | bool] = None,
                 labeller: Optional[Labeller | str | bool] = None,
                 template: Optional[TemplateType] = None):
        """ Parameters passed as None default to Node type's default values. """
        labeller = labeller if labeller is not None else partial(type(self).default_labeller, accessor=accessor)
...
class CompositeNode(BaseNode, abc.ABC):
    ...
    default_child_node_type: type[FieldNode] = FieldNode  # "naked" children are wrapped in this Node type.

    def __init__(self, *children: BaseNode | AccessorSpec,
                 formatter: Optional[Formatter | bool] = None,          # default: use class default_formatter
                 labeller: Optional[Labeller | str | bool] = None,      # default: use class default_labeller
...
MappingElement: TypeAlias = Mapping[Hashable, BaseNode | AccessorSpec] | tuple[Hashable, BaseNode | AccessorSpec]
...
class DictCompositeNode(CompositeNode, Mapping):
    ...
    def __init__(self, *children: MappingElement, **kwargs):
        ...
        mappings = (child if hasattr(child, 'items') else dict((child,)) for child in children)
